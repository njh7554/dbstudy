...
SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME, PHONE_NUMBER
  FROM EMPLOYEES
 WHERE PHONE_NUMBER LIKE '515%'; -- PHONE_NUMBER NOT LIKE '515%'; 515를 제외한 번호가 나옴. 글자수가 명확할 때 _을 써서 제한을 둘 수 있다.
 
-- 8. EMPLOYEES 테이블을 FIRST_NAME의 가나다순(오름차순:Ascending Sort)으로 정렬해서 조회하기.
SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME
  FROM EMPLOYEES
 ORDER BY FIRST_NAME; -- ASC는 생략 가능하다.

-- 9. EMPLOYEES 테이블을 높은 SALARY(내림차순 : Desending Sort)를 받는 사원순으로 먼저 조회하기.
SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME, SALARY
  FROM EMPLOYEES
 ORDER BY SALARY DESC; --DESC는 생략이 불가능하다.
 
-- 10. EMPLOYEES 테이블에서 사원들을 DEPARTMENT_ID순으로 조회하고, 동일한 DEPARTMENT_ID를 가진 사원들은 높은 SALARY순으로 조회하시오.
