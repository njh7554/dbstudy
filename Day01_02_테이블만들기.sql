...
CREATE TABLE PRODUCT(
    ...
    CATEGORY     VARCHAR2(5 BYTE)  CHECK(CATEGORY = 'MAIN' OR CATEGORY = 'SUB'), -- CHECK(CATEGORY IN('MAIN', 'SUB')) 권장 문법
    PRICE        NUMBER            CHECK(PRICE >= 0),
    AMOUNT       NUMBER(2)         CHECK(AMOUNT >= 0 AND AMOUNT <= 100),         -- CHECK(AMOUNT BETWEEN 0 AND 100)   권장 문법
    MANUFACTURED DATE
);
