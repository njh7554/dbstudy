...
SELECT EMP_NO, NAME, SALARY, POSITION, HIRE_DATE, GENDER, DEPART
  FROM EMPLOYEE_TBL
 WHERE SALARY = (SELECT MAX(SALARY)
                   FROM EMPLOYEE_TBL);

-- 4. 평균 급여 이상을 받는 사원을 조회하시오
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, SALARY, HIRE_DATE
  FROM EMPLOYEE_TBL
 WHERE SALARY > (SELECT AVG(SALARY)
                  FROM EMPLOYEE_TBL);
                
-- 5. 평균 근속 개월 수 이상을 근무한 사원을 조회하시오.
SELECT EMP_NO, NAME, DEPART, GENDER, POSITION, HIRE_DATE
  FROM EMPLOYEE_TBL
...
SELECT EMP_NO, NAME, DEPART, POSITION, GENDER, SALARY, HIRE_DATE
  FROM EMPLOYEE_TBL
...
SELECT EMP_NO, NAME, DEPART, POSITION, GENDER, SALARY, HIRE_DATE
 ...
 WHERE DEPART IN ( SELECT DEPT_NO
                     FROM DEPARTMENT_TBL
                    WHERE DEPT_NAME = '영업부'); -- WHERE절에서 사용한 DEPART_NAME 칼럼은 PK/UNIQUE 아니므로 다중 행 서브쿼리이다.

-- 참고. 조인으로 풀기
SELECT E.EMP_NO, E.NAME, E.DEPART, E.POSITION, E.GENDER, E.SALARY, E.HIRE_DATE
...
SELECT DEPT_NO, DEPT_NAME, LOCATION
 ...
 WHERE DEPT_NO IN (SELECT DEPART
                     FROM EMPLOYEE_TBL
                    WHERE POSITION = '과장'); -- WHERE절에서 사용한 POSITION이 칼럼이 PK/UNIQE가 아니므로 다중 행 서브쿼리
-- 참고. 조인으로 풀기
...
-- 실무에선 사용이 불가능하기 때문에 대신 인덱스(INDEX)
SELECT EMP_NO, NAME
  FROM EMPLOYEE_TBL
 WHERE ROWID = 'AAAFFOAABAAALDBAAA';
...
--      3) 원하는 행 번호를 조회한다.
-- 1) ROWNUM 칼럼 사용하기
SELECT EMP_NO, E.NAME, E.SALARY, E.POSITION, E.HIRE_DATE, E.GENDER, E.DEPART
  FROM (SELECT ROWNUM AS RN, A.EMP_NO, A.NAME, A.SALARY, A.POSITION, A.HIRE_DATE, A.GENDER, A.DEPART
          FROM (SELECT EMP_NO, NAME, SALARY, POSITION, HIRE_DATE, GENDER, DEPART
                  FROM EMPLOYEE_TBL
...
SELECT EMP_NO, E.NAME, E.SALARY, E.POSITION, E.HIRE_DATE, E.GENDER, E.DEPART
  FROM (SELECT ROW_NUMBER() OVER(ORDER BY SALARY DESC) AS RN, EMP_NO, NAME, SALARY, POSITION, HIRE_DATE, GENDER, DEPART
          FROM EMPLOYEE_TBL) E 
 WHERE E.RN = 3;
 
-- 3 ~ 4번째로 입사한 사원을 조회하시오.
SELECT E.EMP_NO, E.NAME, E.POSITION, E.HIRE_DATE, E.GENDER, E.DEPART
  FROM (SELECT ROW_NUMBER() OVER(ORDER BY HIRE_DATE ASC) AS RN, EMP_NO, NAME, POSITION, HIRE_DATE, GENDER, DEPART
          FROM EMPLOYEE_TBL) E
...
-- 부서번호가 1인 부서의 사원번호, 사원명, 부서번호, 부서명을 조회하시오.
SELECT
       E.EMP_NO
     , E.NAME
     , E.DEPART
