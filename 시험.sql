...
CREATE VIEW V_PRODUCT
    AS (SELECT P.PROD_NAME, PROD_PRICE
          FROM PRODUCT P);
...
-- 문제2
SELECT EMPID, EMPNAME, SALARY, DEPT_NAME, DEPTID
  FROM EMPLOYEE
 WHERE DEPT_NAME IN (SELECT DEPTID, SALARY, EMPNAME
                       FROM DEPARTMENT
                      WHERE DEPT_NAME = '총무부');
...
CREATE TABLE BOARD_TBL (
    ...
    TITLE         VARCHAR(1000 BYTE) NOT NULL,
    CONTENT       VARCHAR(4000 BYTE) NULL,
    HIT           NUMBER             NULL,
