...
-- 시퀀스(번호 생성기) 만들기 (PK 만들 때 주로 사용한다.)
CREATE SEQUENCE DEPARTMENT_SEQ
    INCREMENT BY 1 -- 1씩 증가하는 번호를 만든다.(생략 가능)
    START WITH 1   -- 1부터 번호를 만든다.(생략 가능)
    NOMAXVALUE     -- 번호의 상환선이 없다.(생략 가능) MAXVALUE 100 : 번호를 100까지만 생성한다.
    NOMINVALUE     -- 번호의 하한선이 없다.(생략 가능) MINVAKYE 100 : 번호의 최소값이 100이다.
...
-- 2. 부서번호(DEPART)가 1인 부서에 근무하는 사원들의 급여를 500000만원 증가시키시오.
UPDATE EMPLOYEE_TBL
