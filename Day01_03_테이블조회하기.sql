/*
    ...
    5. 형식([]는 생략 가능)
        ...
        FROM 조회할 테이블의 이름 (FROM 절)
        [WHERE 조건식]
        [GROUP BY 그룹화할칼럼]  [HAVING 그룹조건식] 예)남자와 여자가 있는 그룹에 여자만 보고 싶을 때 조건식을 작성
        [ORDER BY 정렬할칼럼 정렬방식]
...
/*
    ...
    3. 모든 세부 작업이 성공하면 COMMIT하고 하나라도 실패하면 모든 세부 작업의 취소를 진헹한다.
        (All or Nothing)
*/
...
-- 4) 테이블 별명 지정하기
SELECT E.ENAME
  FROM EMP E;  -- EMP 테이블의 별명을 E로 부여한다. AS(ALIAS)를 사용 할 수 없다.

-- 5) 칼럼 별명 지정하기 
SELECT E.ENAME AS 사원명 -- E.NAME 칼럼의 별명을 '사원명'으로 부여한다. AS(ALIAS)를 사용 할 수 있다.
  FROM EMP E;

-- 2. 사원 테이블의 모든 칼럼 조회하기 
-- 1) * 활용하기(SELECT 절에서 *는 모든 칼럼을 의미한다.)
